...
foreach ($comp in $computers){
    Get-EventLog -computename $comp -LogName Security | where {$events -contains $_.EventID } | select EventID,MachineName,index,EntryType,Message,Source,TimeGenerated | Export-csv -Path "$psscriptroot\$date.csv" -Append
